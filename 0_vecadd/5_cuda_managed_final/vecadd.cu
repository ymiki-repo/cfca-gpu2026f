...
#pragma omp parallel for
  ...
  printf("elapsed time (GPU, 1st time): %e [s] (%e times faster)\n", elapsed_gpu, elapsed_cpu / elapsed_gpu);
  cudaDeviceSynchronize();
  clock_gettime(CLOCK_MONOTONIC, &ini);
  vecadd_gpu<<<NBLOCKS(num, NTHREADS), NTHREADS>>>(num, val, a0, b0, c1);
  cudaDeviceSynchronize();
  clock_gettime(CLOCK_MONOTONIC, &end);
  const double elapsed_gpu_2nd = (double)(end.tv_sec - ini.tv_sec) + (double)(end.tv_nsec - ini.tv_nsec) * 1e-9;
  printf("elapsed time (GPU, 2nd time): %e [s] (%e times faster)\n", elapsed_gpu_2nd, elapsed_cpu / elapsed_gpu_2nd);
